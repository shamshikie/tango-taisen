...
```json
  "ConnectionStrings": {
    "DefaultConnection": "Host=localhost;Database=LearningWordsOnline;Username=postgres;Password=yourpassword"
  }
```
...
初期データを登録します。接続情報はご自身の環境に合わせて変更してください。

```bash
psql -h localhost -U postgres -d LearningWordsOnline -f Data/seed.sql
```
